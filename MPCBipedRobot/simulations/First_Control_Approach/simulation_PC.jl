...
save_dir = project_dir * "./Plot/First_Control_Approach/Simu_PC/" # Directory (relative to project root) for saving simulation plots
...
# Low level controller : define PID structure and initialise integral term to zero  
∫edt = zeros(length(configuration(rs.state)))
pid  = ZMProbot.define_PID(Kp, Ki, Kd, ∫edt)
...
for t in 0.0:br.Ts:tend-br.Ts 
    ...
    # Simulate robot dynamics using computed joint references and PID control
    controller! = ZMProbot.position_controller!(
        rs, br.Ts, t, q, q̇, q̈, Δt, pid, Bu, endEffector;
        use_control = ctrl
    )
...
for k in 1:size(p_foot1, 2)
    pf1 = p_foot1[:, k]
    pf2 = p_foot2[:, k]
    if (pf1[3] <= 0 && pf2[3] <= 0) # DSP 
...
plot!(plt_zmp2, tplot, zmplim[1, :]; linestyle=:dash, color=:red, label="Sim. stability area")
plot!(plt_zmp2, tplot, zmplim[2, :]; linestyle=:dash, color=:red, label=nothing)
