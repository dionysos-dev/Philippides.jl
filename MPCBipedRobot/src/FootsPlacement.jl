...
A Foot Planner as defined in sec. 2 of : 
    1.  Khusainov R, Sagitov A, Klimchik A, Magid E. Arbitrary Trajectory Foot Planner for Bipedal Walking: 
        ...
        Available at: http://www.scitepress.org/DigitalLibrary/Link.aspx?doi=10.5220/0006442504170424
...
Constructor 
"""
function FootPlanner(; br::BipedRobot, check::Bool = false, save::Bool=false, savePath::String="")
    ...
    if (check)
        ...
        plt_traj = plot(; ylims = (1.154, 1.206), title = "Path Planning", xlabel = "X[m]", ylabel = "Y[m]", dpi = 600)
        plot!(plt_traj, xpath, ypath; label = "Reference path")
        display(plt_traj)

        plt = plot(; title = "Foot Planner", xlabel = "X[m]", ylabel = "Y[m]", dpi = 600)

        plot!(plt, xpath, ypath; label = "Reference path")
        ...
        display(plt)
        if save
            savefig(plt, savePath*"Footstep_planner.png")
            savefig(plt_traj, savePath*"path_planner.png")
        end 
    end
...
function nextPoint(
    ...
    direction = 1
    while (L0 >= 0 - e)
        if (L0 <= 0 + e && L0 >= 0 - e)     # if within the tol compute the new orientation
            ...
            Δ = (θ - θ_p)
            if (Δ > pi)
                Δ = 2 * pi - Δ       # Brieuc: pas censé être delta - 2pi ? pas capté ce step 
            elseif (Δ < -pi)
                Δ = 2 * pi + Δ
            end
            if (Δ > 0)
