...
function SwingFootTrajectory(;
    ...
    if (check)
        ...
        display(plt)
        if save 
            savefig(plt, savePath*"plt_swingFootTrajectory.png")
        end 
    end
...
function computeSwingFootTrajectory(br::BipedRobot, fp::FootPlanner, zt::ZMPTrajectory)
    ...
    for stepNum in 1:length(ZMP)               # ZMP is a vector of vector, therefore, length gives number of step (see construction)
